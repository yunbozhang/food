 ...
 * 
 */
class ModuleService extends RecordObject {
    ...
    public static function cacheStrategy($param1,$param2)
    {
    	if((TABLE_CACHE == 1) && (SessionHolder::get('page/status') == 'view') && empty($_GET['_v']))
    	{
    		...
    		if(strcmp($param1,"SELECT * FROM `{$prefix}module_services` WHERE `s_pos` =? AND (`s_locale`=? OR `s_locale`='_ALL') AND (`s_query_hash`=? OR `s_query_hash`='_ALL') AND for_roles LIKE ? AND (`module`<>? OR `action`<>?) ORDER BY `i_order`") == 0)
    		{
    			if(empty(self::$cache_select_data))
    			{
    				if(self::$cache_select_handle)
    				{
    					...
    					$sql1 = "SELECT * FROM `{$prefix}module_services` WHERE (`s_locale`='{$param2[1]}' OR `s_locale`='_ALL') AND (`s_query_hash`='{$param2[2]}' OR `s_query_hash`='_ALL') AND for_roles LIKE '{$param2[3]}' AND (`module`<>'{$param2[4]}' OR `action`<>'{$param2[5]}') ORDER BY `i_order`";
    					$rs =& $db->query($sql1);
    					$objects =& $rs->fetchObjects('ModuleService',array(false, false));
    		...
    		elseif(strcmp($param1,"SELECT COUNT(*) FROM `{$prefix}module_services` WHERE `s_pos`=? AND (`s_locale`=? OR `s_locale`='_ALL') AND (`s_query_hash`=? OR `s_query_hash`='_ALL') AND for_roles LIKE ? AND (`module`<>? OR `action`<>?)") == 0)
    		{
    			$sql = <<<SQL
SELECT COUNT(*) FROM `{$prefix}module_services` WHERE `s_pos`='$param2[0]' AND (`s_locale`='$param2[1]' OR `s_locale`='_ALL') AND (`s_query_hash`='$param2[2]' OR `s_query_hash`='_ALL') AND for_roles LIKE '$param2[3]' AND (`module`<>'$param2[4]' OR `action`<>'$param2[5]') 
SQL;
				$getObjects = Memorycache::FetchMemory($sql,"{$prefix}module_services","count",null);
    		}
    		else
    		{
    			$getObjects = "notmatch";
